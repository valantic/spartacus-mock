...
import { activeTabItems, product } from './mock-data/products/product';
...
import { productReviews } from './mock-data/products/product-reviews';
import { productVariants } from './mock-data/products/product-variants';
import { authRevoke, authToken } from './mock-data/auth/auth';
import { user } from './mock-data/auth/user';
import { addToCart, addVoucher, deleteCart, deleteVoucher, getCart, updateCart } from './mock-data/commerce/cart';
...
export function getDefaultHandlers (environment: Environment): RestHandler[] {
  ...
  return [
    ...
    // TODO is that needed
    /*rest.get(routes.currentCountry, (_req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      return res(ctx.status(200), ctx.json('/ch/de/'));
    }),*/
    ...
    rest.get(routes.product, (req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      ...
      if (requestFields.indexOf('variantOptions') > -1) {
        ...
        return res(ctx.status(200), ctx.json(productVariants()));
      } else {
        // all other scopes
    ...
    rest.get(routes.cart, (req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      ...
      return res(ctx.status(200), ctx.json(getCart(cartId)));
    }),

    // initial cart post call to get an empty cart with a cart id
    rest.get(routes.carts, (_req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      return res(ctx.status(201), ctx.json(getCart('')));
    }),
    ...
    rest.post(routes.addEntries, async (req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      ...
      return res(ctx.status(200), ctx.json(addToCart(product, quantity)));
    }),
    ...
    rest.patch(routes.updateEntries, async (req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      ...
      return res(ctx.status(200), ctx.json(updateCart(cartId, entryNumber, quantity)));
    }),

    // cart delete call to update entries in the cart
    rest.get(routes.removeEntries, (_req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      deleteCart();
      return res(ctx.status(200), ctx.json({}));
    }),

    rest.get(routes.cartVoucher, (req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
    ...
    rest.get(routes.cartVoucherRemove, (_req: RestRequest, res: ResponseComposition, ctx: RestContext) => {
      deleteVoucher();
      return res(ctx.status(200), ctx.json({}));
    }),
